...
create index idx_pet_invites_code on pet_invites(code);
...
-- Indexes
create index idx_timeline_events_pet_date on timeline_events(pet_id, event_date desc);
create index idx_timeline_events_type on timeline_events(event_type);
create index idx_pet_shares_user on pet_shares(user_id);
...
create policy "invites_read_by_code" on public.pet_invites
  for select using (true);  -- anyone with the code can read it to accept
create policy "invites_update_accept" on public.pet_invites
  for update using (accepted_by is null);  -- can only accept unclaimed invites
